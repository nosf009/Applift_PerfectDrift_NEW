...
namespace DavidJalbert
{
    public class TinyCarVisuals : MonoBehaviour
    {
        ...
        public List<GameObject> carMeshParents = new List<GameObject>();

        public void EnableCarMesh(int index)
        {
            if (index >= carMeshParents.Count)
            {
                return;
            }
            foreach (GameObject g in carMeshParents)
            {
                g.SetActive(false);
            }
            carMeshParents[index].SetActive(true);
        }

        public GameObject GetCarGameObjectByIndex(int index)
        {
            return carMeshParents[index];
        }
        ...
        void FixedUpdate()
        {
            ...
            foreach (Transform t in wheelsFront)
            {
                ...
                //skidmarks.AddSkidMark(t.position, Vector3.zero, Color.black, 0);
            }
            ...
            if (particlesDrifting != null && particlesDrifting2 != null)
            {
                if (HCFW.GameManager.Instance)
                {
                    if (HCFW.GameManager.Instance.isSkidding)
                    {
                        ...
                        if (!particlesDrifting2.isPlaying)
                        {
                            particlesDrifting2.Play();
                        }
                    }
                    else
                    {
                        particlesDrifting.Stop();
                        particlesDrifting2.Stop();
                    }
                }
            ...
            {
                if (carController.hasHitSide(minSideCollisionForce))
                {
            ...
            {
                if (carController.hasHitGround(minLandingForce))
                {
            ...
            {
                if (carController.isHittingSide() && carController.getGroundVelocity() > minSideFrictionVelocity)
                {
